...
int main(int argc, char** argv)
{
	...
	//counter to keep track of how many indexes there are
	int cnt = 0;
	// get the books
	...
	auto update = [usdToCadRate, gbpToCadRate](Book& obj) {
		if (obj.country() == "US")
			obj.price() = obj.price() * usdToCadRate;


		if (obj.country() == "UK" && 1990 <= obj.year() && obj.year() <= 1999)
	...
	for (auto i = 3; i < argc; ++i)
	{
		...
		try {
			...
			catch (std::invalid_argument& badfile) {
				std::cout << "** EXCEPTION: " << badfile.what() << std::endl;
			}
	}
	if (argc < 3) {
